...
export default async function PublicCardPage({params}:Props){
  ...
  return <main><article className="publicCard previewShell"><div className="phone"><div className="avatar">FS</div><div className="eyebrow" style={{marginTop:22}}>FS SOFTWARES · VERIFIED PROFILE</div><h1>{card.display_name}</h1><p>{card.title}<br/><strong>{card.company}</strong></p><p>{card.bio}</p><div className="actions">{primary.url&&<a className="btn btnPrimary" href={primary.url}>{primary.label}</a>}<a className="btn" href={`/api/cards/${card.id}/vcard`}>Save contact</a><a className="btn" href={`/api/cards/${card.id}/qr`}>Show QR</a></div><div className="linkList">{links.rows.map(l=><a className="linkButton" href={l.url} key={l.id} rel="noopener noreferrer">{l.label}<span>↗</span></a>)}</div><div className="identityMeta">{phone&&<span>{phone}</span>}{email&&<span>{email}</span>}{location&&<span>{location}</span>}</div><LeadCapture slug={params.slug}/></div></article></main>
}
